...
After taking several measurements, sensing simultaneously with several sensors, or
some other similar operation, weights will be applied using a weighting mechanism to a
preselected set of measurements which will then be aggregated with any desired voting
mechanism.
...
performing a real measurements.
...
behalf of all voters who transfer their power to the proxy.
...
Additionally, since measurements will have unfortunately incurred their cost since in
order for them to have a preference they need to be taken, this paper will not be
able to take full advantage of relieving individual agents of some of their cost.
However, by using less accurate measurement techniques with proxy voting, the hope
is to increase the overall system accuracy without incurring costs of using more
accurate measurement techniques.
Regardless, one question addressed in this paper is if proxy voting should be used at
all, or if direct voting should be used instead by simply averaging the less expensive
measurements.
...
This study aims to explore the idea of using proxy voting as a technique remove the
...
This paper also determines if using such mechanisms is even worthwhile, or if it
