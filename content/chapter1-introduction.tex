...
\begin{itemize}
    \item Mean
    \item {
        Weight by Ranking, where the system-assigned weight of an active voter is
        ...
        If more than one voter receives the same amount of weight, such as when two
        active voters are not delegated any additional weight, they will receive the
        same rank and so the same amount of weight from the system.
        Each active voter is then assigned a new weight from 1 to $n$ according to its
        ranking, where $n$ is the number of distinct delegate-assigned weights.
        This means $n$ can range from 1, when all voters have the same weight, up to
        the number of distinct agents.
        The active voter in first will receive a system-assigned weight of $n$,
        regardless of its total delegator weight, and the active voter in last will
        receive a system-assigned weight of 1.
        The purpose of this mechanism is to offset the effect of super voters by
        reducing the discrepancy between the weights of the proxies with the most and
...
As an example, we'll use the Weight by Ranking mechanism.
...
Since there are 3 distinct weight values, $b$ receives a system-assigned weight of 3,
$a$ receives a weight of 2, and $c$ receives a weight of 1.
From here, the system would calculate the weighted mean of the preferences of $a$,
$b$, and $c$ using these system-assigned weights.
...
% MDH: See the following paragraph. Would it be beneficial to merge them to make
%   the motivation more apparent?
...
We also assume proxy delegations occur after all deliberations for the current round of
