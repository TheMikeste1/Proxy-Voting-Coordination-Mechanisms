...
\vicki{If this is the model, there is less reason to pick a proxy.  They can  just vote remotely.}
...
voting.
\vicki{The reader isn't sure where the error is coming from as voting remotely seems to be the same as being there.  Explain.}
...
\vicki{Explain that initially each player gets one vote.  Explain the term voting power.}
...
\vicki{You appear to be saying that accuracy and error are the same, but they seem opposite.}
...
\vicki{Again, we need to understand how the proxy system works to see why they wouldn't vote the same.}
...
\vicki{Plurality doesn't make sense in a metric space.}while mean takes all preferences, multiplies
...
proxy voting is typically used and is not particularly useful. \vicki{Explain what you mean by useful.}
...
|vicki{Lead with what others have done. Then in a sentence or two say why yours will differ.}
...
In their work, they discover that the Schulze
\vicki{Briefly explain these voting mechanisms} and Minimax voting mechanisms always
agree and beat all other mechanisms tested, while Borda beats 
\vicki{Quantify "beats"} Copeland and Plurality,
and Plurality comes in last.
Our work will differ significantly from theirs, however, as we will explore voting
mechanisms used under proxy voting instead of single-winner elections
\vicki{Why does proxy have to differ from single-winner?}, and most of
...
He treated proxy delegations as a graph \vicki{Explain what the nodes are?  the edges?}, and developed two algorithms to determine
the weights of each proxy.
The first algorithm, which calls for simply dividing their weight \vicki{equally?} among all those
...
\vicki{Are we assuming equal weight?  If not, two delegates would be sufficient as you can get any point in between by proper weighting.

One concern is that if we allow multiple proxies, but they have to be of equal weight, it gets complicated for the delegator to figure out.
}  
...
\vicki{Previous sentence is awkward}
...
\begin{table}[!htbp]
    ...
    \centering
    \input{content/figures/chapter1/distributions-used.tex}
\end{table}
