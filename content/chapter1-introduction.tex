...
\vicki{The reader is unclear why proxy voting would make any difference. This needs
to be clarified early in the paper. We need to understand the rules, as it seems like
the vote is identical to being present to vote.}
...
\vicki{We need to understand the negatives. The way it is described, it seems to be
only a good solution.}
...
the actual preferences of the voters. \vicki{Again, we need to understand the rules
...
\begin{figure}[htbp]
    ...
    \caption{
        ...
        \vicki{Are they ambivalent or just prefer a middle value? Suppose we were
        voting on measures to control immigration. A vote in the middle might not
        mean I'm ambivalent, but that I see the middle approach as the best.}
    }
...
In our implementation it will select the preference of the voter with the most
weight,  \vicki{Unclear. It sounds like you only look at the single votger with the
most weigh.}
...
\vicki{Give us some examples of voting systems that use the continuous preferences.
It seems that if there were just yay/nay votes, proxy votes wouldn't be as
problematic. More previous work would be good here. It would also give you ideas
going forward.}
...
preferences.
\vicki{If we have a range space of voting possibilities, why would we need to pick
the preference of a single proxy? I'm not understanding the motivation.}
\begin{itemize}
    \item Median
