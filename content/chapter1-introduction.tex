...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[scale=7.0]
        ...
        \tkzLabelPoint[above](agent2){$\truthof{\agent_2}$}
        \tkzLabelPoint[above](agent3){$\truthof{\agent_3}$}
        \tkzLabelPoint[above](agent4){$\truthof{\agent_4}$}

        \foreach \n in {agent1, agent2, agent3, agent4}
        \node at (\n)[circle,fill,inner sep=1.75pt]{};


        % Actual preference
        \draw[color=blue, line width=0.5mm, dotted]
        (0.1875, 4pt) -- (0.1875, -3pt);
        \node[color=blue] at (0.1875,-4pt) {$\systemtruth_{actual}$};

        % Preference under proxy vote
        \draw[color=orange, line width=0.5mm, dotted]
        (0.5, 4pt) -- (0.5, -3pt);
        \node[color=orange] at (0.5,-4pt) {$\systemtruth_{proxy}$};
    \end{tikzpicture}

    \caption{
        An example vote and its results.
        $\systemtruth_{actual}$ is the result when everyone votes,
        and $\systemtruth_{proxy}$ is when $\agent_2$ and $\agent_3$ delegate their
        vote and make $\agent_4$ a super voter.
    }
