...
In a vote, each voter starts with one vote.\vicki{awkward}
This vote can be called the voter's \textit{weight} or \textit{voting power}. \vicki{Can we say their weight is one?}
...
\vicki{The reader is unclear why proxy voting would make any difference. This needs to be clarified early in the paper.  We need to understand the rules, as it seems like the vote is identical to being present to vote.}
...
preference space.  \vicki{This seems obvious.}
...
\vicki{We need to understand the negatives.  The way it is described, it seems to be only a good solution.}
...
the actual preferences of the voters. \vicki{Again, we need to understand the rules of proxy voting.  Is it that the active voter must vote the same way for all of the delegated votes?  Is it that the active voter must vote on amendments to the vote, which cannot be known in advance?}
...
extend the results of this study to liquid democracy as well.  \vicki{This could be described in a future work section, but seems to have no purpose here.}
...
\begin{figure}[htbp]
    ...
    \caption{
        ...
        \vicki{Are they ambivalent or just prefer a middle value?  Suppose we were voting on measures to control immigration.  A vote in the middle might not mean I'm ambivalent, but that I see the middle approach as the best.}
    }
...
In our implementation it will select the preference of the voter with the most weight,  \vicki{Unclear.  It sounds like you only look at the single votger with the most weigh.}
...
\vicki{Give us some examples of voting systems that use the continuous preferences.  It seems that if there were just yay/nay votes, proxy votes wouldn't be as problematic.  More previous work would be good here.  It would also give you ideas going forward.}
...
\begin{itemize}
 ...
 Where are you getting this definition?  What motivates it?  I don't see any references where you give credit for this definition.}
    ...
    \item {
        ...
        \vicki{Again, this isn't the typical definition of ranked choice.  Normally it is the candidates that are listed in order (for each voter).}
    }
...
preferences.
\vicki{If we have a range space of voting possibilities, why would we need to pick the preference of a single proxy?  I'm not understanding the motivation.}
\begin{itemize}
    \item Median
...
\vicki{I agree we need this definition of proxy, but we need to motivate it better.  Basically, we are saying we are making this assumption because the way it is really done has no problems.  It would be better to say, we make this assumption because it us more adaptable (like handling ammendments).  Finding previous work which motivates/justifies our assumptions would be good.}
...
reasonable.
\vicki{Rather than "perfect", could we say "reasonable"?  If amendments are being made and discussions are held, opinions will change.  That is a good thing, in reaching consensus.}
...
later. \vicki{Do we need this?  This seems harsh.  What if a voter is in an accident on the way to the House?}
...
They discover that the GT~method~\cite{Rivest2010} beats all others,\vicki{Explain the abbreviation GT}
...
\vicki{You seem to be thinking of the vote as really yay/nay, but that you are allowed to show how strongly you feel about either choice.  This isn't consistent with the presentation earlier.  I'm not sure the voter would do well by voting honestly,  For example, suppose I'm voting between Biden and Trump.  I don't like either, but a slightly prefer Biden.  If I vote slightly towards Biden, I allow my vote to essentially be ignored.}
