...
\thispagestyle{empty}
%%%%%%%%%%%%%
\vicki{Make sure the wording is different from the abstract.}
...
to vote continue operating\vicki{awk}  while still allowing members to quarantine and
...
Second, we assume that each voter has perfect knowledge of potential proxies' opinions. \vicki{So must the proxy vote as the voter wants?  If so, it doesn't matter what the proxy's opinion is.  Rather, it seems the voter just changes the weight of the proxy - the voter and the proxy can't vote differently.}
...
the choice of proxy.
This is differs \vicki{awk} from the actual system presently used, which includes restrictions
...
\vicki{Need previous work section}
...
\vicki{Is there a clearer name?  I see them called principal or elector, but no name really grabs me.  I've also seen designator and designate.  Those terms seem clearer.}
...
still having their voice heard by allowing a proxy to perform the work once \vicki{awk} 
...
\vicki{Look at the references.  Germany, for example, is in lower case (even though the .bib file doesn't have it that way.}
