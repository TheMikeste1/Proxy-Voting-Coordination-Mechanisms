...
Alternatively, if the majority of the options \vicki}options or votes?}  are about the center, it may be a sign
...
\begin{enumerate}
    \item {
        ...
        \systemproxies.
        There is an edge case when two agents share a border \vicki{Unclear what is meant by share a border}  with half the total weight.
...
\end{enumerate}
Additionally, we will apply these mechanisms against  \vicki{use these mechanisms to compare to } all active voters (to simulate
...
\begin{enumerate}
    ...
    \item {
        ...
        weight \vicki{Proxy only votes for themselves.  Allowed to apply is unclear.}, leaving its constituents unrepresented.
    ...
    \item {
        ...
        The group allocates its weight to the median of the proxy's and its
        constituents' preferences.
        However, preferences have changed due to deliberation. \vicki{when you say preferences have changed, do you mean for all voters?  How did they change if they aren't there to hear deliberations?
    }
\end{enumerate}
