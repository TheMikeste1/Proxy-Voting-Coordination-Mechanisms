...
The median mechanism is the same as the one used in~\cite{Cohensius2017},
and is described as (with some symbolic changes)
\begin{equation}
    \textbf{md}(\agents) =
    \min\left\{\agent_i \in \agents \text{ s.t. }
...
For weighted scenarios, this agent will be the minimum agent whose sum of
weights below plus itself is greater than or equal to $\frac{\systemweight}{2}$,
as described with $\textbf{md}(\agents) = \min\left\{
    \agent_i \in \agents \text{ s.t. }
        \weightof{\agent_i} +
