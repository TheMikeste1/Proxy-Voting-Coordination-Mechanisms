...
The median mechanism is the same as the one used in~\cite[para.~2.4]{Cohensius2017},
and is described, with some symbolic changes, as
\begin{equation*}
    \textbf{md}(\systemagents) =
        \min\left\{\agent_i \in \systemagents \text{ s.t. }
...
\begin{equation}
    \textbf{md}(\systemagents) = \min\left\{
        \agent_i \in \systemagents \text{ s.t. }
        \weightof{\agent_i} +
            \sum_{\agent_j \in \systemagents}^{\agent_{i - 1}} \weightof{\agent_j}
        \geq \frac{\systemweight}{2}
\right\}
        \text{,}\label{eq:median-mechanism}
\end{equation}
where \systemagents\ is ordered by \agenttruth.

\mechanismheader{Plurality}
The plurality mechanism selects the proxy with the highest weight.
It is described by
\begin{equation}
    \textbf{pl}(\systemagents) =
    \argmax \weightof{\systemagents}
    \text{,}\label{eq:plurality-mechanism}
\end{equation}
meaning the agent with the maximum weight.
\mechanismheader{Borda}
...
\mechanismheader{Instant Runoff}
...
The weighted instant runoff mechanism is the same as instant runoff, but uses
...
% - Explain what weighting Mechanisms are
%
