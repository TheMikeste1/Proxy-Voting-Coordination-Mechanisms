...
Once an agent has calculated the negative distance for all proxies,
each weight needs to be adjusted to be positive using the formula
\begin{equation}
    g_\agent{(\proxy)} = \lvert
    \min{\weight_{\agent}{(\systemproxies)}} - \weight_{\agent}{(\proxy)}
    \rvert
...
equal to the distance between the minimum and the maximum weight, and also
means the weights fulfill the positive weight requirement.
