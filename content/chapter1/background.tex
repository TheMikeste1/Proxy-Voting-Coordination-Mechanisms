...
a \textit{proxy} \vicki{a person authorized to act for another}.
...
Often \vicki{typically|, this number is one.
These votes are called a voter's \textit{weight} or \textit{voting power} \vicki{For example, an individual with a weight of n who votes for  x has the same effect as if n separate voters picked x.} .
...
These costs may be tangible, such as needing to pay for gas, or intangible, such as \vicki{the time}
...
In this `single-vote' \vicki{Single vote seems confusing as we just told them voting power meant having a larger number of votes.  Could we call this 'unified vote'?} model, an agent must choose a proxy they are confident would
...
% TODO: Why are we using single vote? Then reinforce why use proxy voting
In this study, we will tackle some of the problems associated with single-vote proxy
voting.
By determining methods to minimize the error produced by the model, the system will
yield the benefits of single-vote deliberation while still producing a near-perfect
result.
