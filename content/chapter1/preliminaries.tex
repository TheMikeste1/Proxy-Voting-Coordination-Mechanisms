...
Not all ways of voting are susceptible to such attacks, but it must be a
...
As an example, shareholders at some company are voting on new data collection
policies.
...
\begin{enumerate}
    \item {
        ...
        \vicki{This definition seems weird to me.  What is j?  Why not start at 1 and sum from 1 to i?  Why go to a(i-1) and then add i?}
        ...
        total weight.
        In this case both the lower and higher weighted medians  \vicki{Awkward, lower weighted median isn't really a thing.}, as well as any value
    ...
    \item {
        ...
        $\input{common/figures/equations/weighted-mean}$.  \vicki{This is awkward too.  You sometimes use S and sometimes Sp, but you don't need both. S is sufficient, as the ordering doesn't matter.}
...
\end{enumerate}
We will additionally\vicki{Not sure what is meant by additionally.  Were you meaning that the voting mechanism was used for some other reason?}  apply these voting mechanisms with all active voters (to simulate
...
\vicki{Consider removing the "no preference change" to the first four, including preference change for the last four.  The reader isn't ready for the preference change idea, as it hasn't been introduced, so it is just confusing.}
\begin{enumerate}
    \item {
    ...
    \item {
        \textit{No preference change - Active Only}.  \vicki{The previous sentence indicates these govern how groups coordinate.  This doesn't really meet that criteria. 
 The name also seems odd. }
    ...
    \item {
        ...
        Note this is analogous to only having the active agents vote.  \vicki{Not if the proxy doesn't change their vote.  I think you have to handle this case because otherwise, it makes no sense to have a new name.  }
    }
    \item {
