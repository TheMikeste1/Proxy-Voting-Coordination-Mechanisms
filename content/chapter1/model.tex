...
\vicki{Do we assume that distance measures difference? For example, if my preference is .1, would I like .2 the same as 0?  This isn't always true of single peaked preference, which may be more ordinal than cardinal.}
...
\vicki{But this blurs the next example where amount of money is the issue. Could we say that people in the middle aren't opposed to having pineapple, but wouldn't choose pineapple if they were the only one eating it?  A "don't put pineapple on it for me" attitude.}
...
\vicki{Make it clear than averaging is only one option.}
...
\begin{enumerate}
    \item {
        \textit{No preference change - Selfish}. \vicki{Perhaps dictator?}
