...
extremely far in either direction and so potentially biasing the vote.  \vicki{But this would make no difference if you were using median}
...
\begin{figure}[htbp]
    ...
    \caption{
        ...
        Importantly, points towards the center of the space are the most ambivalent, \vicki{or simply see a compromise solution}
...
Using the interval $[-1, 1]$, we can place `no pineapple' at -1, and `yes pineapple \vicki{Picking a more serious example would be helpful as we need to convince the reader this is an important problem.  Plus, pineapple could always be placed on half the pizza}
at 1.
...
\begin{enumerate}
    ...
    \item {
        ...
        $\input{common/figures/equations/weighted-midrange}$, meaning the weighted
