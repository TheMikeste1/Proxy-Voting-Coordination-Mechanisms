...
\vicki{It would be helpful to compare your results with what other researchers found.}
...
\vicki{Explain why this happens. Explain figure 3.1. I'm assuming the colors don't mean
anything?}
...
\vicki{Lots of zeroes in comparison. Even the non-zeroes are very near zero. Maybe I
...
\vicki{Is there a way to show all on same graph? Hard to compare between figures.}
...
\end{figure}
\vicki{Why is 31 the highest? I'm not sure exactly how your test is set up. How much
...
averaging.  \vicki{Why is this? If voters values are picked from a uniform
...
and \betadistribution{0.3}{0.3} as the inactive distributions.  \vicki{Has this beta
...
\vicki{What is meant by 'placed with itself'?
...
\vicki{This doesn't appear to match your previous discussion where you list all the
...
However, this benefit reduces over time \vicki{It isn't really over time is it? It
...
\end{figure}
\vicki{Hard to read all the different colors. Maybe also use dotted/dashed line types.}
...
Additionally, there seems to be small `hiccups' at each whole number.  \vicki{I don't
...
\begin{figure}[htbp]
    ...
    \label{fig:ratios}
\end{figure}
\vicki{I don't understand why the weighlessaverageAll is so erratic and poor.
